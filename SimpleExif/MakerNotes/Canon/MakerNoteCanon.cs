...
namespace SimpleExif.MakerNotes.Canon
{
    internal partial class MakerNoteCanon : MakerNote
    {
        ...
        Func<int, object, string> actGetSpecialTagValue = GetSpecialTagValue;

        public MakerNoteCanon(FileStream fs)
            : base() {
            this.fs = fs;
        }
        ...
        /// 解析每個Entry的名稱
        /// </summary>
        private void ParseIFDName() {
            var removeEntries = new List<IFDEntry>();
            foreach (var entry in TempMakerNoteIFD.IFDEntries) {
                entry.ViewName = GetDefaultTagName(entry.Tag);
                if (RemoveTagList.Contains(entry.Tag)) {
                    removeEntries.Add(entry);
                }
            }
            var ary = new IFDEntry[removeEntries.Count];
            removeEntries.CopyTo(ary);
            foreach (var entry in ary) {
                TempMakerNoteIFD.IFDEntries.Remove(entry);
            }
        }
        ...
        /// <summary>
        /// 解析每個Entry實際的值(ViewValue)
        /// </summary>
        private void ParseIFDViewValue() {
            foreach (IFDEntry entry in TempMakerNoteIFD.IFDEntries) {
                ...
                switch (entry.Tag) {
                    ...
                    case 0x4018: //LightingOpt
                        ParseSpecial((int[])entry.RealVal, entry.Tag);
                        break;
                    case 0x0025: //FaceDetect2，是byte[]
                        ParseSpecial((byte[])entry.RealVal, entry.Tag);
                        break;
                    ...
                    case 0x0098: { //CropInfo
                            int[] val = (int[])entry.RealVal;
                            AddIFDEntry("CropLeftMargin", val[0].ToString());
                            AddIFDEntry("CropRightMargin", val[1].ToString());
                            AddIFDEntry("CropTopMargin", val[2].ToString());
                            AddIFDEntry("CropBottomMargin", val[3].ToString());
                            break;
                        }
                    default: {
                            entry.ViewValue = Tools.Bin.GetEntryViewValue(entry, actGetSpecialTagValue);
                            AddIFDEntry(entry);
                            break;
                        }
                }
        ...
        /// 最後清除沒有TagName的Entry
        /// </summary>
        private void RemoveEmptyTagName() {
            var removeAry = new List<IFDEntry>();
            foreach (var entry in MakerNoteIFD.IFDEntries) {
                if (string.IsNullOrWhiteSpace(entry.ViewName)) {
                    removeAry.Add(entry);
                }
            }
            var ary = new IFDEntry[removeAry.Count];
            removeAry.CopyTo(ary);
            foreach (var entry in removeAry) {
                MakerNoteIFD.IFDEntries.Remove(entry);
            }
        }
        ...
        /// <summary>
        /// 增加真實Entry到MakerNoteIFD內
        /// </summary>
        /// <param name="ViewName"></param>
        /// <param name="ViewValue"></param>
        private void AddIFDEntry(string ViewName, string ViewValue) {
            IFDEntry entry = new IFDEntry() { ViewName = ViewName, ViewValue = ViewValue };
            MakerNoteIFD.IFDEntries.Add(entry);
        }
        ...
        private void AddIFDEntry(IFDEntry entry) {
            MakerNoteIFD.IFDEntries.Add(entry);
        }
        #endregion
        ...
        internal static string GetSpecialTagValue(int tag, object args) {
            ...
            switch (tag) {
                ...
                case 0x000c: { //Camera Serial Number
                        ulong val = Tools.Calc.IntToULong(((int[])args)[0]);
                        ret = val.ToString();
                        break;
                    }
                ...
                case 0x001e: { //FirmwareRevision
                        string val = Tools.String.FillChar(string.Format("{0:X}", ((int[])args)[0]), 8, "0");
                ...
                case 0x0023: { //Categories
                        int[] val = (int[])args;
                        string by = Tools.String.FillChar(Convert.ToString(val[1], 2), 8, "0");
                        int idx = 0;
